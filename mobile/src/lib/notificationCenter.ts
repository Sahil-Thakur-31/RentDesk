...
const persistReadIds = () => {
  AsyncStorage.setItem(READ_STORAGE_KEY, JSON.stringify(Array.from(readIds))).catch(() => {});
  readListeners.forEach((listener) => listener());
};
...
export const useNotificationsFeed = (properties: any[], portfolio: any) => {
        ...
        const buckets = await Promise.all(
          properties.map(async (property: any) => {
            const [rentRes, utilityRes, tenantRes, paymentRes, maintenanceRes] = await Promise.all([
              api.get(`/properties/${property._id}/rent-records`, { params: { month, year, status: 'unpaid,partial,paid' } }),
              api.get(`/properties/${property._id}/utility-bills`, { params: { month: currentMonth, status: 'unpaid,partial,paid' } }),
              api.get(`/properties/${property._id}/tenants`, { params: { status: 'active' } }),
              api.get(`/properties/${property._id}/payments`, { params: { startDate: start, endDate: end } }),
