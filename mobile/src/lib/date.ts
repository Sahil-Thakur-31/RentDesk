import { getCurrentLocale } from '../context/I18nContext';
...
export const getMonthParts = (monthKey: string) => {
  const [year, month] = monthKey.split('-').map(Number);
  return { year, month };
};

export const getCurrentDateValue = () => new Date().toISOString().slice(0, 10);
